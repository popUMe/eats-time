<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<link rel="icon" href="https://www.kurly.com//favicon.ico" />
<link rel="stylesheet" href="${pageContext.request.contextPath}/static/css/all.css">
<link rel="stylesheet" href="${pageContext.request.contextPath}/static/css/findIdOk.css">
<title>컬리 - 마켓컬리/뷰티컬리</title>
</head>
<body>
	<div class="css-140915z">
		<div class="css-6mht10">
			<div class="css-1ymrnwr">
				고객님의 컬리 계정을 찾았습니다.<br>(db결과따라 달라짐)
				<div class="css-1jlgal3">아이디 확인 후 로그인해 주세요.<br>(db결과따라 달라짐)</div>
			</div>
			<ul class="css-14o5m2r ewnnxio4">
				<li class="css-1qp8f6i"><img
					class="css-ate73y"
					src="data:image/svg+xml;base64,"
					alt="정보보기">
				<div class="css-yzpdz">
						<div class="css-1kkr6j1">아이디(db결과따라 달라짐)</div>
						<div class="css-1081952">가입일(db결과따라 달라짐)</div>
					</div></li>
			</ul>
			<div class="css-gyfngb">
				<button class="css-ufulao" type="button">
					<span class="css-ymwvow">아이디 전체보기</span>
				</button>
				<button class="css-1qirdbn" type="button">
					<span class="css-ymwvow">로그인</span>
				</button>
			</div>
		</div>
	</div>
</body>
<script src="${pageContext.request.contextPath}/static/js/findIdOk.js"></script>
</html>
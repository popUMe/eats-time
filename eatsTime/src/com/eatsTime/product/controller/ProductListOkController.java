...
import com.eatsTime.product.domain.Search;

public class ProductListOkController implements Action {
	...
	public Result execute(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		Criteria criteria = new Criteria(page, productDAO.getTotal(pagable), sort, category);
		...
		criteria = new Criteria(page, productDAO.getTotal(pagable), sort, category);

		productDAO.selectAll(pagable).stream().map(product -> new JSONObject(product)).forEach(jsonArray::put);

		req.setAttribute("realEndPage", criteria.getRealEndPage());
		req.setAttribute("product", jsonArray.toString());
		req.setAttribute("total", productDAO.getTotal(pagable));
